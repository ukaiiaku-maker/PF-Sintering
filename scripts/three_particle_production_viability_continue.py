...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--ratio',type=float,required=True);ap.add_argument('--outer-nm',type=float,required=True);args=ap.parse_args()
    ...
    out=source.parent/f'continuation_{args.ratio:.2f}_{args.outer_nm:.3f}';out.mkdir(exist_ok=False)
    def row():
        return dict(time_s=t*op.physics.seconds_per_model_time,contacts=evaluate_contacts(f,op,m),
                    ...
                    mass_relative_error=float(grain_volumes(f,g).sum()/v0.sum()-1),mirror_error=float(np.max(abs(f-f[::-1]))))
    rows.append(row())
    def save():
        report=dict(status=status,ratio=args.ratio,outer_radius_nm=args.outer_nm,history=rows,rejections=rejected,
          wall_s=time.perf_counter()-start,events_enabled=False,stationary_null_is_gate=False,
        ...
        (D/f'continuation_{args.ratio:.2f}_{args.outer_nm:.3f}.json').write_text(json.dumps(report,indent=2)+'\n')
    ...
    while t*op.physics.seconds_per_model_time<30:
        h=min(h,2.,30/op.physics.seconds_per_model_time-t)
        try:
        ...
        if sum(H.values())>=m['root_threshold_multiplier']:status='APPRECIABLE_HAZARD_REQUIRES_EVENT_VALIDATION';break
        if time.perf_counter()-start>900:status='WALL_BUDGET';break
