...
from pf_sintering.three_particle_phase_a import PhaseAOperator,initialize_step,initialization_locality_gate
...
def main():
    ...
    for i in range(args.steps):
        ...
        if i%1000==0 or i==args.steps-1:
            ...
            if not initialization_locality_gate(f,g['f'],g)['pass_locality']:
                print('STOP: initialization generated excessive dilute-vapor material',flush=True)
                break
    ...
    locality=initialization_locality_gate(f,g['f'],g)
    gates=dict(locality=locality['pass_locality'],projected_residual=residual<1e-5,grain_volume=volume_error<1e-9,mirror=row['mirror_error']<1e-10,angle=angle_error<3,topology=not row['topology_stop'])
    ...
    report=dict(status='INITIALIZATION_NUMERICAL_GATES_PASS_REVIEW_CURVATURE' if eligible else 'INITIALIZATION_NOT_QUALIFIED',gates=gates,locality=locality,volume_error=volume_error,angle_error_deg=angle_error,residual=residual,wall_s=time.perf_counter()-start,history=history,final=row)
