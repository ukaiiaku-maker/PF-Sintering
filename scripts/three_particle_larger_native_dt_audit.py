...
class LargerDtEvent(ContactEvent):
 def __init__(self,g,pair):super().__init__(g,pair,max_fast_blocks=1024);self.dt*=2
...
def main():
 ...
 for factor,klass in [(1,BufferedContactEvent),(2,LargerDtEvent)]:
  c,o=compatible_chain(.65,119.999*1e-9);g=map_to_pf(c,o,4e-9,.5e-9);event=klass(g,pair,max_fast_blocks=512) if factor==1 else klass(g,pair);start=time.perf_counter()
  ...
  rows.append(dict(dt_factor=factor,dt_model=event.dt,steps_per_check=10//factor,wall_s=time.perf_counter()-start,completed=result[4],clock_s=result[5]['event_time_model']*MANIFEST['seconds_per_model_time'],metrics=m,f_min=float(result[0].min()),f_max=float(result[0].max())));print('DONE',factor,flush=True)
 reference,candidate=rows;candidate['field_linf_to_reference']=float(np.max(np.abs(fields[1]-fields[0])));candidate['stress_difference_Pa']=candidate['metrics'][contact+'_sigma_local_Pa']-reference['metrics'][contact+'_sigma_local_Pa'];candidate['relative_clock_difference']=candidate['clock_s']/reference['clock_s']-1
 passed=bool(candidate['completed'] and candidate['f_min']>=-1e-8 and candidate['f_max']<=1+1e-8 and candidate['field_linf_to_reference']<2e-8 and abs(candidate['stress_difference_Pa'])<10 and abs(candidate['relative_clock_difference'])<1e-7)
