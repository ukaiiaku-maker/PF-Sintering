...
source=Path('runs/three_particle_volume_loading_065/loss_1pct.npz');D=Path('docs/three_particle/volume_loading_065');m=json.loads(Path('docs/three_particle/production_screen/bicrystal_launch_manifest.json').read_text());rule=json.loads(Path('docs/three_particle/cmc/angle_calibration.json').read_text())['rule']
...
(D/'step_ceiling_predeclared.json').write_text(json.dumps(dict(source=str(source),sha256=hashlib.sha256(source.read_bytes()).hexdigest(),duration_s=5.,ceilings_model=[20.,200.],limits=limits,unchanged_embedded_error_tests=True,discarded_copies_only=True),indent=2)+'\n')
fields=[];rows=[]
for cap in [20.,200.]:
 c,o=compatible_chain(.65,119.999*1e-9);g=map_to_pf(c,o,4e-9,.5e-9);op=PhaseAOperator(g);it=HarmonicSurfaceDiffusion(op)
 with np.load(source) as d:f=d['f'].copy();np.testing.assert_array_equal(g['z'],d['z'])
 v0=grain_volumes(f,g);t=0.;h=20.;end=5/op.physics.seconds_per_model_time;accepted=0;rejected=0;wall=time.perf_counter()
 while t<end-1e-12:
  h=min(h,cap,end-t)
  try:
  ...
  except (FloatingPointError,RuntimeError):rejected+=1;h*=.2;continue
...
(D/'step_ceiling_audit.json').write_text(json.dumps(report,indent=2)+'\n');np.savez_compressed(D/'step_ceiling_audit.npz',fields=np.array(fields));print(report['passed'],diff,flush=True)
