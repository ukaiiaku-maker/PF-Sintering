"""Milestone 13C Sections 7-9: isolated sinusoidal substrate flattening.

The EXACT current substrate profile (wavelength=480nm, amplitude=24nm,
W=20nm) with the particle removed entirely -- X reflecting, Y periodic,
isotropic surface energy, unified conserved surface diffusion only (no
eta at all, since there is only one grain). A hard, independent physical
benchmark: the crest must flatten and material must flow away from it on
both sides.

Tracks A1/A2/A3 (first three Fourier harmonics of the free-surface
X-position as a function of Y), crest/trough position and curvature,
total f mass, and free energy, continuously through the trajectory (not
just endpoints), plus the away-from-crest surface flux on both sides.
"""
...
def run(dx_nm, args, target_times):
    ...
    print(f"=== isolated substrate, dx={dx_nm}nm: Nx={p.Nx} Ny={p.Ny} dt={p.dt:.4e}s wavelength={wavelength*1e9:.2f}nm ===")
    ...
    def sample(step, t):
        ...
        return dict(step=step, time_s=t, A1=amps[0], A2=amps[1], A3=amps[2],
    ...
    for t_target in target_times:
        ...
        print(f"  t={step*p.dt:.4e}s A1={row['A1']*1e9:.4f}nm A2={row['A2']*1e9:.4f}nm A3={row['A3']*1e9:.4f}nm "
              ...
              f"F={row['F']:.6e} mass_drift={row['mass_drift']:.2e}")

    return dict(dx_nm=dx_nm, wavelength=wavelength, amplitude0=amplitude0, p_dt=p.dt, rows=rows)
...
def main():
    ...
    ap.add_argument("--out", type=str, default="runs/m13c_isolated_substrate.json")
    args = ap.parse_args()
