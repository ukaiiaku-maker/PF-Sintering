...
def main():
 global OUT
 ap=argparse.ArgumentParser();ap.add_argument('--resume',action='store_true');ap.add_argument('--out',type=Path,default=OUT);ap.add_argument('--source',type=Path,default=SOURCE);ap.add_argument('--history-source',type=Path);ap.add_argument('--full-jacobian',action='store_true');ap.add_argument('--reuse-preconditioner',action='store_true');ap.add_argument('--max-step-model',type=float,default=20.);ap.add_argument('--disable-reflection-guard',action='store_true');args=ap.parse_args();OUT=args.out;source=args.source
 ...
 with np.load(OUT/'checkpoint.npz' if args.resume else source) as d:
  assert not bool(d['events_enabled']);np.testing.assert_array_equal(g['z'],d['z']);np.testing.assert_array_equal(g['gb'],d['gb'])
  ownership=d['ownership'].copy()
  if np.max(abs(ownership-g['ownership']))>5e-15:raise ValueError('source ownership is not the selected three-particle geometry')
  g['ownership']=ownership;f=d['f'].copy();t=float(d['t_model']);h=float(d['next_h']) if 'next_h' in d else 2.
 op=PhaseAOperator(g)
 ...
 if args.resume:
  launch=json.loads((OUT/'launch.json').read_text());assert launch.get('solver','frozen_mobility_jacobian')==engine and launch.get('reuse_preconditioner',False)==args.reuse_preconditioner and launch['max_step_model']==args.max_step_model and launch.get('reflection_guard_enabled',True)==(not args.disable_reflection_guard)
 ...
 while status=='RUNNING':
  ...
  try:
   ...
   trial_volumes=grain_volumes(trial,g);reason=trial_invariant_reason(trial,float(trial_volumes.sum()),float(v0.sum()),enforce_reflection=not args.disable_reflection_guard)
  ...
  elif abs(now['mass_relative_error'])>1e-11:status='CONSERVATION_GUARD'
  elif not args.disable_reflection_guard and now['mirror_error']>1e-8:status='CONSERVATION_SYMMETRY_GUARD'
