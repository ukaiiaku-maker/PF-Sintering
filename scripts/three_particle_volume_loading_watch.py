...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--run',type=Path,required=True);ap.add_argument('--poll-seconds',type=float,default=30.);args=ap.parse_args()
 os.chdir(ROOT);run=args.run;out=Path('docs/three_particle/volume_loading_065/automatic_snapshots');out.mkdir(parents=True,exist_ok=True)
 while True:
  ...
  if tag is not None and not (out/tag).exists():
   ...
   if coherent:
    ...
    assert validation['maximum_mass_error']<=1e-11 and validation['maximum_mirror_error']<=1e-8 and validation['energy_monotone']
