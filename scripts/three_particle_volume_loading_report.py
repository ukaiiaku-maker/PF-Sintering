...
from pf_sintering.three_particle_loading import decompose
...
summary=dict(source_run=str(R),status=report['status'],events_enabled=False,thresholds_drawn=False,baseline=launch['baseline'],derivative_definition='Centered secant over 0.05 percentage points of volume loss, excluding x<0.1%; diagnostic only, never an activation correction.',derivative=derivative,checkpoints=checkpoints,plateau=report['plateau'],final=extension[-1],decision='B: clear operational plateau' if report['status']=='STRESS_PLATEAU' else 'Pending trajectory completion and physical review')
