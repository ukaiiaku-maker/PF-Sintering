...
from pf_sintering.three_particle_event import pair_transfer,ownership_pair_step,update_ownership,save_event_checkpoint
...
class ContactEvent:
    ...
    def bind(self,state):
        f=state[0];phi=np.divide(np.array(state[1:]),f[None],out=self.g['ownership'].copy(),where=abs(f[None])>1e-30)
    ...
    def metrics(self,state,q):
        ...
        result=dict(**c,**geometric,r_TJ_m=c['r_n_m'],contact_area_m2=np.pi*c['r_n_m']**2,
            ...
            q_over_b=q,total_volume_m3=float(grain_volumes(f,self.g).sum()),
            V_left_m3=float(grain_volumes(f,self.g)[0]),V_center_m3=float(grain_volumes(f,self.g)[1]),V_right_m3=float(grain_volumes(f,self.g)[2]),
    ...
    def relax(self,state,q):
        ...
        for block in range(1,61):
            ...
            row=self.metrics(current,q);increment=fast_manifold_increment(previous,row)
            ...
            previous=row
        return current,row,dict(converged=False,iterations=600,blocks=60)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--source',type=Path,required=True);ap.add_argument('--out-name',default='forced_left');args=ap.parse_args()
    ...
    g['ownership']=phi;state=(f,*(phi*f[None]));event=ContactEvent(g);before=event.metrics(state,0.);watch0=curvature_watch(f,event.op,two_contact_center=True)
    ...
    result=event.run(state,callback=callback);final=result[:4];complete=result[4];info=result[5];save_event_checkpoint(out/'final.npz',final,info['event_restart'],contact='LEFT',label='FORCED_EVENT_FOR_MECHANICAL_QUALIFICATION');after=event.metrics(final,info['event_progress_over_b']);watch1=curvature_watch(final[0],event.op,two_contact_center=True)
