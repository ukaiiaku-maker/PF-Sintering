...
c,o=compatible_chain(.65,119.999e-9);g=map_to_pf(c,o,4e-9,.5e-9);event=probe.ImplicitFastProbe(g,max_fast_blocks=240);wall=time.perf_counter();result=event.run(state,target=.0025)
...
r=dict(label='OFF_TRAJECTORY_CACHED_IMPLICIT_FAST_PROBE',source=str(source),completed=result[4],stop_reason=result[5]['stop_reason'],stop_detail=result[5].get('stop_detail'),wall_s=time.perf_counter()-wall,fields_Linf_to_native=float(np.max(abs(np.array(result[:4])-native))),sigma_difference_to_native_Pa=metrics['LEFT_sigma_local_Pa']-reference['metrics']['LEFT_sigma_local_Pa'],event_info=result[5],production_promoted=False)
r['event_info']={k:v for k,v in r['event_info'].items() if k!='event_restart'}
Path('docs/three_particle/production_065/fast_implicit_cached_probe.json').write_text(json.dumps(r,indent=2,default=float)+'\n')
np.savez_compressed('runs/three_particle_production_065/fast_implicit_cached_probe.npz',initial=np.array(state),native=native,implicit=np.array(result[:4]));print({k:v for k,v in r.items() if k!='event_info'},flush=True)
