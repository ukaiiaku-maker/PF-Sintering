...
## 3. e1+e2=f consistency audit

Recorded at every active-window step (`e1e2f_residual = max|e1+e2-f|`). Across all 114,310 steps of the corrected run's active window: **max 0.0200, mean 0.0022**. This is a small, pre-existing numerical drift already present in the frozen pre-activation state itself (the prescribed-displacement microtest, Section 5 below, measured `e1e2f_residual=0.0444` at `dz=0`, i.e. before any RBM activity at all) — a generic accumulation from ordinary PF stepping/clipping over ~90,000 steps, not something introduced or worsened by the corrected transport mechanism. Not further investigated given time constraints; flagged as a minor open item.
...
`pf_sintering.m16k_neck_tracking.find_tj_from_contour`: locates the triple junction directly as the point along the already-traced free-surface contour (`f=0.5`) where grain identity flips (`e1-e2` changes sign), rather than relying solely on a local minimum of `R(z)` (an indirect geometric proxy). Wired into the corrected driver's diagnostics (`z_tj_from_contour_nm`, `tj_agreement_nm` columns in `history.csv`) as a cross-check, not a replacement, of the `R(z)`-minimum-based tracker — given time constraints, the two were not yet reconciled into a single authoritative contact-position estimate.
