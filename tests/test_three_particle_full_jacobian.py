...
def test_full_jacobian_step_consistency_mass_and_reflection(reuse):
 f,op,_=fixture(mirror=True);it=FullJacobianSurfaceDiffusion(op,reuse_preconditioner=reuse);F=it.rhs(f);h=1e-8;a=it.step(f,h);b=it.step(f,h/2)
